...
void	Bureaucrat::executeForm(AForm const & src) {
	if (!src.getSigned())
		std::cout << this->_name << " couldn't execute form " << src.getName() << " because form is not signed"<< std::endl;
	else if (this->_grade > src.getGradeExec())
		std::cout << this->_name << " couldn't execute form " << src.getName() << " because grade's too low"<< std::endl;
	else
